...
const hasCriticalStock = computed(() => {
  return filteredStockList.value.some((item) => {
    ...
    return !isNaN(warehouseAmount) && warehouseAmount < 150
  })
})
  ...
  >
    <template #prepend>
      <div class="d-flex align-center gap-2">
        <v-chip prepend-icon="mdi-palette" color="primary" variant="tonal">
          Boya Stok Listesi
        </v-chip>
        <v-chip
      ...
      <v-table class="h-100" fixed-header hover>
        ...
        <tbody>
          <template v-for="(item, index) in filteredStockList" :key="item.id">
            <tr
              :class="{
                'bg-error text-white': Number(item.warehouseStockAmount) < 150
              }"
            ...
            >
              <td>
                <div class="d-flex align-center gap-2">
                  <v-icon
                    v-if="Number(item.warehouseStockAmount) < 150"
                    color="white"
                    size="small"
                    icon="mdi-alert"
                  />
              ...
              <td :class="{ 'font-weight-bold': Number(item.warehouseStockAmount) < 150 }">
