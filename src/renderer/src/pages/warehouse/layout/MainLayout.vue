...
const checkUnverifiedLabel = async () => {
  labelService.getUnverifiedLabel().then((res) => {
    const { datNo } = res?.data?.data ?? {}

    if (datNo) {
      store.dispatch('production/activeLabelVerification', { datNo })
    }
  })
}
